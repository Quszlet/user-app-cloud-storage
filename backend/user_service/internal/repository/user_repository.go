...
func (up *UserMySql) Create(user models.User) (int, error) {
	var id int
	query := fmt.Sprintf("INSERT INTO %s (login, email, password, status) values ($1, $2, $3, $4) RETURNING id", usersTable)

	row := up.db.QueryRow(query, user.Login, user.Email, user.Password, user.Status)
